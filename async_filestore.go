...
type asyncFileStoreFactory struct {
	settings *Settings
}
...
func NewAsyncFileStoreFactory(settings *Settings) MessageStoreFactory {
	return asyncFileStoreFactory{settings: settings}
}

// Create creates a new FileStore implementation of the MessageStore interface
func (f asyncFileStoreFactory) Create(sessionID SessionID) (msgStore MessageStore, err error) {
...
func (store *asyncFileStore) Reset() error {
...
func (store *asyncFileStore) Refresh() (err error) {
	if err = store.cache.Reset(); err != nil {
...
func (store *asyncFileStore) populateCache() (creationTimePopulated bool, err error) {
...
func (store *asyncFileStore) setSession() error {
...
func (store *asyncFileStore) setSeqNum(f *os.File, seqNum int) error {
...
func (store *asyncFileStore) NextSenderMsgSeqNum() int {
	return store.cache.NextSenderMsgSeqNum()
}

// NextTargetMsgSeqNum returns the next MsgSeqNum that should be received
func (store *asyncFileStore) NextTargetMsgSeqNum() int {
	return store.cache.NextTargetMsgSeqNum()
}

// SetNextSenderMsgSeqNum sets the next MsgSeqNum that will be sent
func (store *asyncFileStore) SetNextSenderMsgSeqNum(next int) error {
...
func (store *asyncFileStore) SetNextTargetMsgSeqNum(next int) error {
...
func (store *asyncFileStore) IncrNextSenderMsgSeqNum() error {
...
func (store *asyncFileStore) IncrNextTargetMsgSeqNum() error {
...
func (store *asyncFileStore) CreationTime() time.Time {
...
func (store *asyncFileStore) getMessage(seqNum int) (msg []byte, found bool, err error) {
...
func (store *asyncFileStore) GetMessages(beginSeqNum, endSeqNum int) ([][]byte, error) {
	var msgs [][]byte
